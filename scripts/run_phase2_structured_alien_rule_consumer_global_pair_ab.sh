...
# Runs the Stage-1 global-only rule-consumer smoke A/B for a single seed on top
# of the current replay-off development line:
# 1) no_prio                      -> current development baseline
# 2) no_prio_rule_consumer_global -> same baseline + global-only rule consumer
#
# Intended usage:
#   SEED=4 GPU_ID=0 PAIR_PARALLEL=2 TRAIN_STEPS=20000 \
#   BASE_LOGDIR=/path/to/logdir/rule_consumer_global_seed4 \
#   scripts/run_phase2_structured_alien_rule_consumer_global_pair_ab.sh
...
RUN_NAME=${RUN_NAME:-bench_atari_structured_20k_alien_rule_consumer_global_pair_seed${SEED}_$(logdir_run_tag)}
...
print_git_run_metadata "$ROOT_DIR" "rule-consumer-global pair seed ${SEED} launch"

if [ "$PAIR_PARALLEL" -ge 2 ]; then
  run_variant no_prio &
  pid_a=$!
  run_variant no_prio_rule_consumer_global \
