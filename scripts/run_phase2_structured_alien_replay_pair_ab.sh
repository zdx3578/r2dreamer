...
# Runs the 50k Alien replay pair A/B for a single seed:
# 1) current_head -> current repo defaults
# 2) no_prio      -> same config + buffer.prioritized=False
#
...
#   BASE_LOGDIR=/path/to/logdir/replay_pair_seed4 \
#   scripts/run_phase2_structured_alien_replay_pair_ab.sh
#
...
RUN_NAME=${RUN_NAME:-bench_atari_structured_50k_alien_replay_pair_seed${SEED}_$(logdir_run_tag)}
...
print_git_run_metadata "$ROOT_DIR" "replay pair seed ${SEED} launch"

if [ "$PAIR_PARALLEL" -ge 2 ]; then
  run_variant current_head &
  pid_a=$!
  run_variant no_prio buffer.prioritized=False &
  pid_b=$!
  wait "$pid_a"
  wait "$pid_b"
else
