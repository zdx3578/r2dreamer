...
# Runs the Stage-1 global-only + late-enable + sticky-gate-ramp rule-consumer
# smoke A/B for a single seed on top of the replay-off development line:
# 1) no_prio
# 2) no_prio_rule_consumer_global_sticky_ramp
#
# Compared to v6, this v7 keeps sticky gate enable but smooths the post-latch
# takeover with a short ramp to avoid abrupt late-stage overcorrection.
...
RUN_NAME=${RUN_NAME:-bench_atari_structured_20k_alien_rule_consumer_global_sticky_ramp_pair_seed${SEED}_$(logdir_run_tag)}
BASE_LOGDIR=${BASE_LOGDIR:-"$ROOT_DIR/logdir/$RUN_NAME"}
GATE_THRESHOLD=${GATE_THRESHOLD:-0.06}
LATCH_RAMP_UPDATES=${LATCH_RAMP_UPDATES:-125}
...
print_git_run_metadata "$ROOT_DIR" "rule-consumer-global-sticky-ramp pair seed ${SEED} launch"
...
if [ "$PAIR_PARALLEL" -ge 2 ]; then
  run_variant no_prio &
  pid_a=$!
  run_variant no_prio_rule_consumer_global_sticky_ramp "${STICKY_ARGS[@]}" &
  pid_b=$!
  wait "$pid_a"
  wait "$pid_b"
else
