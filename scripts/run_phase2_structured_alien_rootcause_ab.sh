...
# Runs the seed {1,2} Alien 50k root-cause A/B matrix used to separate:
# 1) current_head
# 2) direct_target_blend=0.0
# 3) prioritized replay off
# 4) direct_target_blend=0.0 + prioritized replay off
#
# Each variant reuses the standard repro runner, so training, latest.pt probing,
# and local summaries stay identical to the existing workflow.
#
# Typical usage:
#   MACHINE_NAME=$(hostname -s) GPU_ID=0 MAX_PARALLEL=2 SEEDS="1 2" \
#   scripts/run_phase2_structured_alien_rootcause_ab.sh
...
SEEDS=${SEEDS:-"1 2"}
RUN_NAME=${RUN_NAME:-bench_atari_structured_50k_alien_rootcause_ab_$(logdir_run_tag)}
...
print_git_run_metadata "$ROOT_DIR" "rootcause ab launch"

run_variant current_head

run_variant \
  blend0 \
  model.phase1a.direct_target_blend=0.0
