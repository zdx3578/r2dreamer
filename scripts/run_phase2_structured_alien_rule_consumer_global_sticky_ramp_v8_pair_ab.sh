...
# Runs the Stage-1 v8 global-only + sticky-gate-ramp rule-consumer smoke A/B
# for a single seed on top of the replay-off development line:
# 1) no_prio
# 2) no_prio_rule_consumer_global_sticky_ramp_v8
#
# Compared to v7, v8 keeps the same global-only sticky gated consumer but moves
# the latch threshold earlier and slows the post-latch ramp:
# - gate_threshold: 0.045
# - latch_ramp_updates: 250
#
# The goal is to let the consumer become active earlier on hard seeds such as
# seed_4 without reintroducing the late-stage overcorrection seen before v7.
...
RUN_NAME=${RUN_NAME:-bench_atari_structured_20k_alien_rule_consumer_global_sticky_ramp_v8_pair_seed${SEED}_$(logdir_run_tag)}
...
print_git_run_metadata "$ROOT_DIR" "rule-consumer-global-sticky-ramp-v8 pair seed ${SEED} launch"
...
if [ "$PAIR_PARALLEL" -ge 2 ]; then
  run_variant no_prio &
  pid_a=$!
  run_variant no_prio_rule_consumer_global_sticky_ramp_v8 "${STICKY_ARGS[@]}" &
  pid_b=$!
  wait "$pid_a"
  wait "$pid_b"
else
