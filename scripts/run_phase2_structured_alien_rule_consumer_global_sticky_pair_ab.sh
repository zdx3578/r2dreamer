...
# Runs the Stage-1 global-only + late-enable + sticky-gate rule-consumer smoke
# A/B for a single seed on top of the replay-off development line:
# 1) no_prio
# 2) no_prio_rule_consumer_global_sticky
#
# The consumer stays off until the late schedule is active, and once the
# detached phase2 gate crosses the threshold it remains enabled for the rest of
# the run.
...
RUN_NAME=${RUN_NAME:-bench_atari_structured_20k_alien_rule_consumer_global_sticky_pair_seed${SEED}_$(logdir_run_tag)}
...
print_git_run_metadata "$ROOT_DIR" "rule-consumer-global-sticky pair seed ${SEED} launch"
...
if [ "$PAIR_PARALLEL" -ge 2 ]; then
  run_variant no_prio &
  pid_a=$!
  run_variant no_prio_rule_consumer_global_sticky "${STICKY_ARGS[@]}" &
  pid_b=$!
  wait "$pid_a"
  wait "$pid_b"
else
