...
# Runs the Stage-1 global-only + phase2-gated rule-consumer smoke A/B for a
# single seed on top of the replay-off development line:
# 1) no_prio
# 2) no_prio_rule_consumer_global_gate
#
# This v5 control keeps the late-enable schedule from v4 and adds a detached
# phase2-gate threshold. The default threshold is tuned to hold the consumer
# off until phase2 gate scale is meaningfully non-trivial on the 20k run.
...
RUN_NAME=${RUN_NAME:-bench_atari_structured_20k_alien_rule_consumer_global_gate_pair_seed${SEED}_$(logdir_run_tag)}
BASE_LOGDIR=${BASE_LOGDIR:-"$ROOT_DIR/logdir/$RUN_NAME"}
GATE_THRESHOLD=${GATE_THRESHOLD:-0.06}
...
print_git_run_metadata "$ROOT_DIR" "rule-consumer-global-gate pair seed ${SEED} launch"

if [ "$PAIR_PARALLEL" -ge 2 ]; then
  run_variant no_prio &
  pid_a=$!
  run_variant no_prio_rule_consumer_global_gate \
    ...
    model.rule_prediction_consumer.gate_enable_mode=threshold \
